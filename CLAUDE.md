...
Use this checklist as a reference for what good code looks like — it applies whether you're writing or reviewing code. When reviewing a PR specifically, see the `pr-review` skill for scope rules and breaking-change filtering.
...
- [ ] All subsystems initialized in every robot config (SIM, WoodBot, PracticeBot) — use Noop IOs for missing hardware; see the `robot-config-parity-check` skill
