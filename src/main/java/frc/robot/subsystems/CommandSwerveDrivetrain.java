...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
  ...
  private final SwerveRequest.FieldCentricFacingAngle m_faceHubRequest = new SwerveRequest.FieldCentricFacingAngle()
   ...
   * face the hub center. The heading controller automatically adjusts the robot's
   * rotation to
   * always point toward the hub as the robot moves around the field.
   *
   * @param velocityXSupplier Supplier for X velocity (field-relative, forward
   *                          positive) in m/s
   * @param velocityYSupplier Supplier for Y velocity (field-relative, left
   *                          positive) in m/s
  ...
  private final ProfiledPIDController m_headingOverridePID = new ProfiledPIDController(
      HEADING_KP,
      HEADING_KI,
      HEADING_KD,
      new TrapezoidProfile.Constraints(
          maxAngularVelocity.in(RadiansPerSecond),
          maxAngularVelocity.in(RadiansPerSecond) * 4));

  private boolean m_headingOverrideEnabled = false;

  private Rotation2d m_headingOverrideTarget = Rotation2d.kZero;
  ...
  public CommandSwerveDrivetrain(
    ...
    m_headingOverridePID.enableContinuousInput(-Math.PI, Math.PI);
    PPHolonomicDriveController.overrideRotationFeedback(
        () -> {
          if (!m_headingOverrideEnabled) {
            return 0.0;
          }
          return m_headingOverridePID.calculate(
              getStateCopy().Pose.getRotation().getRadians(),
              m_headingOverrideTarget.getRadians());
        });
  }

  public void enableHeadingOverride(Rotation2d target) {
    m_headingOverrideTarget = target;
    if (!m_headingOverrideEnabled) {
      m_headingOverrideEnabled = true;
      m_headingOverridePID.reset(
          getStateCopy().Pose.getRotation().getRadians(),
          getStateCopy().Speeds.omegaRadiansPerSecond);
    }
    m_headingOverrideTarget = target;
  }

  public void disableHeadingOverride() {
    m_headingOverrideEnabled = false;
    PPHolonomicDriveController.clearRotationFeedbackOverride();
  }

  public boolean isAimedAt(Rotation2d target, double toleranceRadians) {
    return Math.abs(
        getStateCopy().Pose.getRotation().minus(target).getRadians()) < toleranceRadians;
  }
